...
```
โจทย์ต้องการให้คำนวณว่ากบจะต้องกระโดดกี่ครั้งเพื่อไปถึงเส้นชัย โดยกบจะกระโดดได้ลดลงทีละ 2 เมตร 

Input: ระยะกระโดดเริ่มต้นและระยะเส้นชัย

Output: จำนวนครั้งที่ต้องกระโดด หากไม่สามารถไปถึงจะตอบ -1

Constraints: กบมีโอกาสที่จะกระโดดไปไม่ถึงเส้นชัย
```
...
```text
Step 1: สร้าง input
Step 2: ตั้งตัวแปรให้ ระยะทาง = 0, จำนวนครั้งที่กระโดด = 0
Step 3: While loop เมื่อระยะทางยังน้อยกว่าเป้าหมาย โดยเพิ่มระยะทางตามระยะกระโดดปัจจุบันแล้ว +1 จำนวนการกระโดด แล้ว -2 ระยะที่กระโดดครั้งต่อไป หากระยะกระโดดครั้งต่อไปจะ <= 0 ให้ break 
Step 4: If else เช็คว่าถ้าระยะทาง >= เป้าหมาย ให้แสดงจำนวนครั้งการกระโดด หากไม่ ให้แสดง -1
```
...
```
วิธีสุดท้ายของฉันเหมือนกับวิธีเดิม เพราะฉันสามารถคิด Testcase ได้หลายแบบขณะคิดแผนแรก ทำให้ไม่ได้ติด Testcase ใดๆ และ code นี้เขียนและเข้าใจได้ง่าย
```
...
```
เช็คว่า code จะสรุปว่ากบกระโดดถึงเส้นชัยหากกบกระโดดเป็นระยะทางเกินเป้าหมาย
```
...
```
เช็คว่า code จะลดระยะกระโดดทุกครั้งทีละ 2 เมตร
```
...
```
เช็คว่า code จะสรุปว่ากบกระโดดถึงเส้นชัยหากกบถึงเป้าหมายแม้ระยะทกระโดดสุดท้ายคือ 0 เมตร
```
...
```
คู่ Pair และเพื่อนในสาขา
```

เขาช่วยอะไร

```
ช่วยทำความเข้าใจโจทย์และคิดวิธีการเขียน code
```
