...
```
โจทย์ต้องการให้สร้างระบบที่คำนวณเงินที่ต้องจ่ายหากส่งพัสดุในน้ำหนักต้องการจากที่หนึ่งไปที่หนึ่ง

Input:
บรรทัดที่ 1: ต้นทาง ปลายทาง
บรรทัดที่ 2: น้ำหนักพัสดุ

Output: จำนวนเงินที่ต้องจ่าย หากเส้นทางไม่ได้อยู่ในตาราง จะแสดงผลว่า "Error"

Constraints: จำนวนข้อมูลในตารางมีมากเกินไป ทำให้ไม่สามารถใช้ฟังก์ชัน if else ธรรมดาได้
```
...
Step 2: สร้าง dictionary ตามตารางที่โจทย์ให้มา โดยดูต้นทางพัสดุก่อน แล้วค่อยดูปลายทางเพื่อดูค่าธรรมเนียม
Step 3: เขียนฟังก์ชัน if else ว่าถ้าหาก input ตรงกับค่าหนึ่งใน dictionary ให้ดึงค่านั้นมาคำนวณและแสดงผล นอกนั้นจะให้คำตอบเป็น "Error"
```
...
```
วิธีสุดท้ายของฉันเหมือนกับแผนแรก เพราะ code ในแผนนี้สามารถทำตามที่โจทย์ต้องการได้ และไม่ได้มีความยาวมากเกินไป
```
...
```
เช็คว่า code แสดงผลเป็น Error หากต้นทางและปลายทางคือที่เดียวกัน
```
...
```
เช็คว่า code สามารถแสดงผลเป็น Error หากต้นทางและปลายทางไม่ตรงกับในตาราง
```
...
```
เช็คสูตรคำนวณค่าธรรมเนียมที่เขียนใน code
```
