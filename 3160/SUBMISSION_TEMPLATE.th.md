...
```
โจทย์ต้องการให้สร้างระบบที่หาจำนวนและจำนวนเฉพาะในช่วงเลขหนึ่ง

Input: เลขเริ่มต้นและเลขตัวสุดท้ายของช่วง

Output:
จำนวนเฉพาะ และจำนวนเลขในช่วงหนึ่ง

Constraints: input สามารถเป็นเลขตัวลบ ซึ่งไม่สามารถเป็นจำนวนเฉพาะได้แม้จะหารลงตัว
```
...
```text
Step 1: สร้าง Input ตามโจทย์
Step 2: สร้างตัวแปรจำนวนของจำนวนเฉพาะ (total) = 0 และ list ว่างไว้ใส่จำนวนเฉพาะ
Step 3: สร้าง for loop เช็คเลขในช่วง input
Step 4: สร้าง for loop ด้านในอีกทีเพื่อไล่เช็คว่าจำนวนนั้น ตั้งแต่หารด้วย 2 จนถึงก่อนถึงตัวมันเองนั้นมีตัวไหนหารลงตัวไหม หากไม่มีจะเป็นจำนวนเฉพาะ แล้วนำใส่ list และ +1 total 
Step 5: แสดงผลตามที่โจทย์กำหนด
```
...
```
วิธีสุดท้ายของฉันเพิ่ม
- เงื่อนไขใน loop นอกว่าถ้าหากเลขมีค่าน้อยกว่า 2 จะข้ามไปเช็คตัวต่อไปเลย
- เงื่อนไขใน loop ในว่าหากเลขตัวนั้นๆ เจอเลขที่หารลงตัวแล้ว จะ break แล้วข้ามไปเช็คเลขตัวต่อไป
- เงื่อนไขสุดท้ายว่าถ้าหากไม่มีจำนวนเฉพาะเลย จะไม่ต้อง print จำนวนเฉพาะที่เช็คเจอ
```
...
```
เช็คว่า code จะไม่ print ว่างบนบรรทัดแสดงจำนวนเฉพาะหากไม่มีจำนวนเฉพาะในช่วง
```
...
```
เช็คว่า code จะไม่นับเลข 1 เป็นจำนวนเฉพาะ
```
...
```
เช็คว่า code จะไม่นับเลขติดลบเป็นจำนวนเฉพาะ
```
...
```
ช่วยให้เห็น concept ของจำนวนเฉพาะ และได้เห็นเงื่อนไขบางส่วนที่ฉันไม่ได้ใส่ใจ
```
