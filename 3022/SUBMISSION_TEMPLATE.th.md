...
```
โจทย์ต้องการให้สร้างระบบที่ช่วยแปลงตัวเลขจากหน่วยอุณหภูมิหนึ่งไปอีกหน่วยหนึ่งตามที่ผู้ใช้กำหนด

Input:
บรรทัดแรก เป็นจำนวนจริง เป็นค่าอุณหภูมิ
บรรทัดที่ 2 เป็นอักษรย่อหน่วยอุณหภูมิ C,F,K,R ที่เป็นหน่วยของตัวเลขในบรรทัดแรก
บรรทัดที่ 3 เป็นอักษรย่อของหน่วยทั้งสี่ ซึ่งเป็นหน่วยของคำตอบที่ต้องการ

Output:
เป็นจำนวนจริงทศนิยม 2 ตำแหน่งซึ่งคำนวณและแปลงหน่วยออกมาแล้ว

Constraints:
จากสูตรการแปลงหน่วยที่โจทย์ให้มา พบว่าสูตรเป็นของแค่หน่วย C เท่านั้น ทำให้ไม่สามารถแปลงหน่วยอุณหภูมิในครั้งเดียวได้โดยตรงในบาง Testcases
```
...
```text
Step 1: สร้าง Input ใส่เลขอุณภูมิ
Step 2: สร้างฟังก์ชัน if else ที่แปลงหน่วยอุณภูมิเดิมเป็นหน่วย Celcius
Step 3: สร้างฟังก์ชัน if else ที่แปลงจากหน่วย Celcius เป็นหน่วยที่ผู้ใช้ต้องการ 
Step 4: แสดงผลคำตอบออกมาในรูปแบบทศนิยม 2 ตำแหน่ง
```
...
```
ฉันใช้วิธีสุดท้ายเหมือนกับที่วางแผนแรกไว้ทุกอย่าง เนื่องจากแผนไม่ได้มีความซับซ้อนและเข้าใจขั้นตอนง่าย รวมถึงไม่ได้ยากต่อการเขียน code 
```
...
```
เช็คว่าระบบจะให้คำตอบถูกหากหน่วยอุณหภูมิเดิมและหน่วยที่ต้องการแปลงเป็นหน่วยเดียวกัน
```
...
```
เช็คว่าระบบเงื่อนไขที่แปลงเป็นหน่วย C ทำงานปกติ และได้คำตอบถูกต้องหากหน่วยเดิมของเลขคือหน่วย C ตั้งแต่แรก
```
...
```
เช็คว่าระบบทำงานได้ปกติหาก input เป็นเลขติดลบ
```
...
```
ได้ร่วมกับคู่ Pair ช่วยหาทางแก้ปัญหาโจทย์เบื้องต้น แต่ไม่ได้แก้โจทย์จนจบในแล็บนั้น และให้พี่ TA ช่วยอ่านและตีความหมายของโจทย์
```

คุณยังทำอะไรด้วยตนเอง

```
ได้คิดแผนแรก การเขียน code และหา errors ทั้งในแล็บและเมื่อนำกลับมาทำต่อด้วยตนเอง สร้างและทดลอง Testcases และส่งงานด้วยตนเอง
```
