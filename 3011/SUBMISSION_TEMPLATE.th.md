...
```text
โจทย์ต้องการให้เขียนระบบที่บอกว่าเมื่อสี 2 สีจากแม่สีมาผสมกัน ผลลัพธ์จะออกมาเป็นสีอะไร

input: ชื่อสี 2 สี จากแม่สี (Red, Yellow, Blue)
output: สีผลลัพธ์ (Orange, Violet, Green)
constraints: สีทั้ง 2 สีที่ผสมกันสามารถเป็นสีเดียวกันได้เนื่องจากโจทย์ไม่ได้จำกัดในเรื่องนี้
```
...
```text
Step 1: input ใส่สี 2 สี
Step 2: ฟังก์ชัน if else ผลลัพธ์ของสีที่ออกมา
Step 3: แสดงผลออกมาเป็นสีผลลัพธ์
```
...
```text
ใช้วิธีสุดท้ายเหมือนกับวิธีแรก เพราะจำนวนสีที่กำหนดมีไม่มาก ทำให้ไม่ได้ลำบากในตอนที่เขียนฟังก์ชัน
```
...
```text
เพื่อเช็คว่าเงื่อนไขที่ผลลัพธ์เป็น Error ทำงานได้ปกติ
```
...
```text
เช็คว่าฟังก์ชันสามารถให้คำตอบที่ถูกต้องหาก input ทั้ง 2 เป็นแม่สีเดียวกัน
```
...
```text
เช็คว่าหาก input ทั้ง 2 เป็นสีเดียวกันแต่ไม่ได้อยู่ในแม่สี คำตอบจะไม่ได้เป็นสีที่เป็น input แต่ให้คำตอบเป็น Error
```
...
```text
ช่วยคิดเงื่อนไขในกรณีที่ input เป็นแม่สีเดียวกันเพื่อแก้ Testcases ที่ผิด
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันวางแผน เขียน code ทั้งหมด สร้าง testcases ใหม่ขึ้นมาและทดลอง รวมถึงส่งงานเอง
```
