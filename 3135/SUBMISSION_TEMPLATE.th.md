...
```
โจทย์ต้องการให้สร้างระบบที่บอกว่าก่อนที่ของขวัญที่วนรอบวงจะถูกขโมย หรือจะวนกลับมาคนที่แรก ผ่านมือคนมาแล้วกี่คน
Input: จำนวนคนในวง (N) จำนวนคนที่ข้ามเมื่อส่งของให้คนต่อไป (K) และลำดับที่นั่งของขโมย (T) ทั้งหมดในบรรทัดเดียว

Output:
จำนวนคนที่ได้พิจารณาของขวัญ

Constraints: การมีตัวแปร K อาจจะที่ให้การคำนวณตำแหน่งของขวัญเกิดตำแหน่งที่เกินจำนวนคนที่มีอยู่
```
...
```text
Step 1: สร้าง Input ตามโจทย์กำหนด
Step 2: ตั้งตัวแปรตำแหน่ง = 1 และจำนวนคนที่ได้ของขวัญ = 0
Step 3: สร้างฟังก์ชัน while loop เพื่อเลื่อนตำแหน่งของขวัญและเพิ่มจำนวนผู้ได้ของขวัญ โดยจะทำต่อไปจนกว่าตำแหน่งจะตรงกับขโมยหรือวนกลับไปคนเดิม
Step 4: เขียน while loop ด้านในเพื่อคำนวณเคสที่เลขตำแหน่งมากกว่าจำนวนคน
Step 5: จบ loop และแสดงผลจำนวนคนที่ได้จับของขวัญ
```
...
```
วิธีสุดท้ายของฉันเปลี่ยนตรงที่
- ตั้งให้จำนวนคนที่ได้ของขวัญ = 1 โดยรวมคนแรก
- เขียนให้ while loop แรกทำงานเรื่อยๆ หากตำแหน่งยังไม่ถึงโจร 
- +1 จำนวนคนที่จับของขวัญหลังจากผ่านเงื่อนไขข้อด้านล่าง
- มีเงื่อนไขว่าหากตำแหน่งถึงโจรแล้ว จะ +1 คนที่ได้ของขวัญแล้ว break หรือ break เลยหากของขวัญวนกลับมาที่คนแรก 
- ไม่มี while loop ด้านในเพื่อคำนวณเคสที่เลขตำแหน่งมากกว่าจำนวนคน ซึ่งเขียนสูตรคำนวณแทน
```
...
```
เช็คว่าเงื่อนไขเคสที่ของวนกลับมาหาคนแรกนั้นทำงานถูกต้อง
```
...
```
เช็คว่าผลลัพธ์จำนวนคนที่ได้จับของขวัญนั้นรวมคนแรกสุดในตอนเริ่มด้วย
```
...
```
เช็คสูตรคำนวณตำแหน่งในเคสที่เลขเกินจำนวนคนนั้นถูกต้อง
```
