...
```
โจทย์ต้องการให้สร้างระบบที่หาจำนวนกำแพงปิรามิดน้อยสุดที่ต้องพังหากต้องการเดินทางจากห้องที่เราอยู่ไปที่ห้องหมายเลข 1

Input: หมายเลขห้องปัจจุบัน

Output: จำนวนกำแพงน้อยสุดที่ต้องพัง

Constraints:
ห้องมีมากถึง 100,000 ห้อง ทำให้ต้องมีสูตรคำนวณบางอย่างที่ทำให้หาคำตอบได้ง่ายขึ้น
...
```text
Step 1: input ใส่หมายเลขห้อง
Step 2: ตั้งค่าของจำนวนกำแพงที่พัง = 0
Step 3: สร้างฟังก์ชัน for loop แบบนับถอยหลัง โดยมีการเขียน if else เพื่อดูว่าจะต้องไปที่ห้องหมายเลขใดต่อและ +1 จำนวนกำแพงที่พังเมื่อย้ายห้อง
Step 4: แสดงผลจำนวนกำแพงที่พังไปทั้งหมด
```
...
```
- วิธีสุดท้ายของฉันคือการใช้สูตรคำนวณหาแถวที่ห้องนั้นๆ อยู่ และใช้ if else คำนวณจำนวนกำแพงที่พัง 
- จะไม่มีการใช้ for loop และการตั้งตัวแปรให้มีค่าเป็น 0 เนื่องจากฉันคิดว่า code จะซับซ้อนเกินไป
```
...
```
เช็คว่าคำตอบจะเป็น 0 หากห้องปัจจุบันเป็นห้องที่ 1 อยู่แล้ว
```
...
```
เช็คว่า code รันได้หาก input มีคำสูงสุด
```
...
```
เช็คว่าได้คำตอบที่ถูกต้องหากห้องปัจจุบันอยู่ริมสุดของแถวนั้นๆ
```
...
```
เขาได้แนะนำสิ่งที่ต้องทำลำดับแรกเพื่อให้ได้คำตอบ นั่นคือการหาตำแหน่งแถวของหมายเลขห้องปัจจุบัน
```

คุณยังทำอะไรด้วยตนเอง

```
ฉันทำความเข้าใจโจทย์ คิดแผนแรก เขียน code เทสคำตอบ ถามคำถาม AI และส่ง code เข้าระบบด้วยตนเองทั้งหมด
```
