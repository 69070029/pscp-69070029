...
```
โจทย์ต้องการให้สร้างระบบที่แสดงผลว่ามีเลขใดหาร 10 ลงตัวบ้าง ตั้งแต่เลขที่เป็น input จนถึง 0

Input: ใส่จำนวนเต็มบวก

Output:
แสดงผลออกมาเป็นจำนวนเต็มบวกตั้งแต่เลข input จนถึง 0 ที่หาร 10 ลงตัว

Constraints: input ไม่จำเป็นต้องหาร 10 ลงตัว ทำให้การเขียน code ในข้อนี้จะซับซ้อนขึ้น
```
...
```text
Step 1: สร้าง Input ใส่จำนวนเต็มบวก
Step 2: สร้างฟังก์ชัน while loop เพื่อหาคำตอบ
Step 3: แสดงผลคำตอบออกมา 
```
...
```
วิธีสุดท้ายของฉันคล้ายกับแผนแรกแค่คร่าวๆ รายละเอียดของ code แตกต่างตรงที่
- ฉันกำหนดให้รับ input เป็น string 
- แล้วเปลี่ยนหลักหน่วยของเลขเป็น 0 และแปลง type ของ input เป็น int ก่อนนำเข้า while loop 
- ซึ่งใน while loop จะลบค่าตัวเลขทีละ 10 จนถึง 0 และแสดงผล
```
...
```
เช็คว่า code สามารถแปลงเลขหลักหน่วยเป็น 0 ได้
```
...
```
เช็คว่า code ทำงานได้ถูกต้อง หากหลักหน่วยของเลข input เป็น 0 ตั้งแต่แรก 
```
...
```
เช็คว่า code ทำงานได้ปกติ หาก input เป็น 0 ตั้งแต่แรก
```
...
```
ได้ร่วมกับคู่ Pair ช่วยหาทางแก้ปัญหาโจทย์และเขียนโปรแกรมทั้งหมด
```
