...
```
โจทย์ต้องการให้สร้างระบบคำนวณเงินที่นักเรียนแต่ละคนต้องจ่ายโดยยึดจากเงื่อนไขที่โจทย์กำหนด

Input:
บรรทัดที่ 1: สถานะสมาชิก (Y/N)
บรรทัดที่ 2: จำนวนสินค้าที่ซื้อ
บรรทัดที่ 3 (2+n): ราคาสินค้าแต่ละชิ้น

Output: ยอดเงินที่ต้องจ่ายหลังหักส่วนลดเป็นทศนิยม 2 ตำแหน่ง

Constraints: โจทย์อยากได้คำตอบที่ปัดทศนิยมแบบ round half up ทำให้ไม่สามารถเขียน format ธรรมดา
```
...
```text
Step 1: สร้าง Input ใส่ข้อมูลตามโจทย์กำหนด
Step 2: ตั้งค่าตัวแปรจำนวนเงินที่ต้องจ่ายเป็น 0
Step 3: เขียนฟังก์ชัน for loop เพื่อคิดยอดรวมสินค้าทั้งหมดและนำไปใส่ในตัวแปรที่สร้างใน Step 2
Step 4: สร้างฟังก์ชัน if else และลดราคาตามกำหนด
Step 5: เขียนเงื่อนไขการปัดเศษทศนิยมในกรณีที่ทศนิยมตำแหน่งที่ 3 เป็น 5 ที่โดยปกติคำตอบจะไม่ตรงกับระบบ round half up
Step 6: แสดงคำตอบด้วย format ทศนิยม 2 ตำแหน่ง
```
...
```
วิธีสุดท้ายของฉันคล้ายกับแผนแรก ต่างตรงเรื่องของการปัดทศนิยม โดยฉันเปลี่ยนมาใช้ library decimal แทนเพื่อลดความซับซ้อน
```
...
```
เช็คว่า code ยังคำนวณราคาได้ หากราคาสินค้าทั้งหมดเป็น 0
```
...
```
เช็คว่า code ให้คำตอบถูกต้อง หากยอดการซื้อเป็น 0 แต่สถานะสมาชิกเป็น Y ซึ่งต้องลดราคา
```
...
```
เช็คการทำงานของ ROUND_HALF_UP หากทศนิยมตำแหน่งที่ 3 คือ 5
```
...
```
ฉันทำความเข้าใจโจทย์ คิดแผนและเขียน code ตรวจสอบ Errors และส่ง code เข้าระบบด้วยตนเอง
```
